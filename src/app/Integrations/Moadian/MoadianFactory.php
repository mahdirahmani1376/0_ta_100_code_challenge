...
class MoadianFactory
{
    ...
    private function createInvoiceHeader(Invoice $invoice): self
    {
        ...
        $postal_code = ($invoice->client->is_legal == 1 && $invoice->client->company_postal_code) ? $invoice->client->company_postal_code : $invoice->client->postal_code;
        $header->bpc = (isset($postal_code) && strlen($postal_code) == 10) ? $postal_code : '1234567890';
        //$header->bpc = ($invoice->client->is_legal == 1 && $invoice->client->company_postal_code) ? $invoice->client->company_postal_code : $invoice->client->postal_code;
    ...
    {
        /** @var Item $item */
        foreach ($invoice->items()->where('amount', '>', 0)->get() as $item) {
            ...
            $body->am = '1';
            $body->mu = 1627;
            $body->fee = round($item->amount);
            $body->prdis = round($item->amount);
            $body->dis = 0;
            $body->adis = round($item->amount);
            $body->vra = 9;
            ...
            $body->tsstam = round($item->amount + $body->vam);
    ...
    private function getMappedProductId(Item $item): array
    {
        ...
        switch ($item->invoiceable_type) {
            ...
            case Item::TYPE_PRODUCT_SERVICE_UPGRADE:
                ...
                if (\Illuminate\Support\Str::contains($product['name'], ['نمایندگی'])) {
                    $code = 2330001496167;
